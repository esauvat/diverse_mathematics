
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                               PREAMBLE
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\documentclass[a4paper, 10pt, twoside]{report}
\usepackage[top = 2.5cm,left=2.54cm,right=2.54cm,bottom=2.54cm]{geometry}
\usepackage[french]{babel}
\usepackage[utf8]{inputenc} 
\usepackage[T1]{fontenc}
\usepackage{lxfonts}
\usepackage[mathcal, mathbf]{euler}
\usepackage[hidelinks]{hyperref}
\usepackage{ulem}
\usepackage{fancyhdr}
\usepackage{shorttoc}

\usepackage{xcolor}
\usepackage{pagecolor}
\usepackage{graphicx}
\usepackage{tikz}
\usepackage{epigraph}
\usepackage{fancyhdr}
\usepackage{blkarray}

\usepackage{amsfonts}
\usepackage{amsmath}
\usepackage{amsthm}
\usepackage{stmaryrd}
\usepackage{multicol}
\usepackage{bbold}
\usepackage{amssymb}
\usepackage{yhmath}
\usepackage{xfrac}
\usepackage{dsfont}
\usepackage{mathrsfs}
\usepackage{cancel}
\usepackage{soul}

\usepackage{shorttoc}
\usepackage{minitoc}

% \renewcommand{\thefootnote}{\fnsymbol{footnote}}

\pagestyle{fancy}
\setlength{\headheight}{13.59999pt}
\fancyhead[LE]{\thepage}
\fancyhead[RE]{\small \leftmark}
\fancyhead[LO]{\small \rightmark}
\fancyhead[RO]{\thepage}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                            Cover Page Details
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\renewcommand\epigraphflush{flushright}
\renewcommand\epigraphsize{\normalsize}
\setlength\epigraphwidth{0.7\textwidth}

\definecolor{titlepagecolor}{cmyk}{1,.60,0,.40}

\DeclareFixedFont{\titlefont}{T1}{ppl}{b}{it}{0.5in}

\makeatletter                       
\def\printauthor{%                  
    {\large \@author}}              
\makeatother
\author{%
    Mis en forme par \\
    Émile Sauvat \\
    \texttt{emile.sauvat@ens.psl.eu}\vspace{20pt}}

% The following code is borrowed from: https://tex.stackexchange.com/a/86310/10898

\newcommand\titlepagedecoration{%
\begin{tikzpicture}[remember picture,overlay,shorten >= -10pt]

\coordinate (aux1) at ([yshift=-15pt]current page.north east);
\coordinate (aux2) at ([yshift=-410pt]current page.north east);
\coordinate (aux3) at ([xshift=-4.5cm]current page.north east);
\coordinate (aux4) at ([yshift=-150pt]current page.north east);

\begin{scope}[titlepagecolor!40,line width=12pt,rounded corners=12pt]
\draw
  (aux1) -- coordinate (a)
  ++(225:5) --
  ++(-45:5.1) coordinate (b);
\draw[shorten <= -10pt]
  (aux3) --
  (a) --
  (aux1);
\draw[opacity=0.6,titlepagecolor,shorten <= -10pt]
  (b) --
  ++(225:2.2) --
  ++(-45:2.2);
\end{scope}
\draw[titlepagecolor,line width=8pt,rounded corners=8pt,shorten <= -10pt]
  (aux4) --
  ++(225:0.8) --
  ++(-45:0.8);
\begin{scope}[titlepagecolor!70,line width=6pt,rounded corners=8pt]
\draw[shorten <= -10pt]
  (aux2) --
  ++(225:3) coordinate[pos=0.45] (c) --
  ++(-45:3.1);
\draw
  (aux2) --
  (c) --
  ++(135:2.5) --
  ++(45:2.5) --
  ++(-45:2.5) coordinate[pos=0.3] (d);   
\draw 
  (d) -- +(45:1);
\end{scope}
\end{tikzpicture}%
}

\begin{document}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                            Command Definitions
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newcommand{\dessin}[2]{
    \begin{center}
        \input{dessins/#1}
        \captionof{figure}{#2}
    \end{center}    }


% %%%    Some command to create theorem and definition easier      %%%

% \theoremstyle{plain}
% \newtheorem{thm}{Théorème}[section]
% \newtheorem{lem}[thm]{Lemme}
% \newtheorem{prop}[thm]{Proposition}
% \newtheorem*{cor}{Corollaire}
% \newcommand{\theorem}[2]{
%     \hspace{15pt} {\vrule width 1pt} \kern2pt
%     	\begin{minipage}[t]{0.9\textwidth}
%             	\begin{#1} ${}$ \vspace*{0.15cm}\\	
%                 #2 
%      		\end{#1}
%      	\end{minipage}
%      \medskip
%     }
%     %commande pour créer un théorème, le premier argument est le type et le second le contenu
%         % ex : \theorem{lem}{contenu du lemme}
    
% \newcommand{\namedtheorem}[3]{
%     \newtheorem*{#3}{#1}
%     \hspace{15pt} {\vrule width 1pt} \kern2pt
%     	\begin{minipage}[t]{0.9\textwidth} 
%             \begin{#3} ${}$ \vspace*{0.15cm}\\      
%                 #2 
%             \end{#3} 
%         \end{minipage}
%     \medskip
%     }

% \theoremstyle{remark}
% \newtheorem*{nd}{Note}
% \newcommand{\remarque}[1]{\begin{nd} #1 \medskip \end{nd}}
%     %commande pour créer une remarque, le contenu passe en argument
%         % ex : \remarque{contenu de la remarque}
% \newtheorem*{rap}{Rappel}
% \newcommand{\rappel}[1]{\begin{rap} #1 \medskip \end{rap}}

% \newcommand{\traitd}{\begin{center} ~ \medskip  \\ \rule{0.9\textwidth}{0.75pt} \vspace{-10pt}\end{center}}
% \newcommand{\trait}{\vspace{-5pt} \begin{center} \rule{0.9\textwidth}{0.75pt} \end{center} ~ }
% \newcommand{\traitdouble}{\trait \vspace{-30pt} \traitd}

% \newcommand{\ard}{\begin{array}{l}}
% \newcommand{\arf}{\end{array}}


% %%%     Maths misc      %%%

% \newcommand{\abs}[1]{\left\vert #1 \right\vert}

% \newcommand{\appli}[4]{
%     \left(
%     \begin{array}{ccccc} 
%         & #1 & \longrightarrow & #3 & \\ 
%         & #2 & \longmapsto & #4 &
%     \end{array}
%     \right)
% }

% \newcommand{\scal}[1]{\left< #1 \right>}

% \newcommand{\ent}[1]{{\llbracket #1 \rrbracket}}                    
% 	% Ensemble d'entiers
% \newcommand{\N}{{\mathbf{N}}}                                       
% 	% Entiers naturels
% \newcommand{\Z}{{\mathbf{Z}}}                                       
% 	% Entiers relatifs
% \newcommand{\R}{{\mathbf{R}}}                                       
% 	% Réels
% \newcommand{\C}{{\mathbf{C}}}                                       
% 	% Complexes
% \newcommand{\K}{{\mathbf{K}}}                                       
% 	% Corps K
% \newcommand{\Q}{{\mathbf{Q}}}                                       
% 	% Rationnels
% \newcommand{\KX}{{\mathbf{K}[X]}}                                   
% 	% Polynômes sur K
% \newcommand{\M}{{\mathcal{M}}}                                      
% 	% Matrices
% \newcommand{\GL}{{\mathcal{G}\hspace*{-0.05cm} \mathcal{L}}}        
% 	% Groupe Linéaire
% \newcommand{\cont}{{\mathcal{C}}}                                   
% 	% Fonctions continues   
% \newcommand{\cpm}{{\mathcal{C}_{pm}^0}}                             
% 	% Fonctions continues par morceaux
% \newcommand{\lin}{{\mathcal{L}}}                                    
% 	% Applications linéaires
% \newcommand{\Sym}{{\mathcal{S}}}                                    
% 	% Matrices symétrique
% \newcommand{\Orth}{{\mathcal{O}}}                                   
% 	% Matrices orthogonales
% \newcommand{\bigo}{{\mathcal{O}}}
% \newcommand{\Part}{\mathcal{P}}                                     
% 	% Parties
% \newcommand{\A}{{\mathscr{A}}}                                      
% 	% Tribu

% \newcommand{\norm}[1]{\left\Vert #1 \right\Vert}
% \newcommand{\normop}[1]{{\left\vert\kern-0.15ex\left\vert\kern-0.15ex\left\vert #1 \right\vert\kern-0.15ex\right\vert\kern-0.15ex\right\vert}}

% \newcommand{\suminf}{\sum_{n=0}^\infty}

% \newcommand{\suite}[1]{(#1_n)}
% \newcommand{\stox}[1]{\underset{x \to #1}{\longrightarrow}}

% %%%     Format      %%%

% \newcommand{\highlight}[1]{\colorbox{lightgray}{#1}}

% \newcommand{\fin}{
% 	~ \vspace{10pt}\\
%     \begin{center}
%         { \LARGE $\star$ \hspace{0.7cm} $\star$ \hspace{0.7cm} $\star$ }
%     \end{center}
% }

\newcommand{\sk}[2]{\sum_{k= #1 }^{#2}}
\newcommand{\si}[2]{\sum_{i= #1 }^{#2}}
\newcommand{\pk}[2]{\prod\_{k= #1 }^{#2}}
\newcommand{\pdi}[2]{\prod_{i= #1 }^{#2}}
\newcommand{\Ps}{\sum_{k=0}^{+\infty} a_k X^k}
\newcommand{\Psn}{\sum_{k=0}^n a_k X^k}
\newcommand{\kx}{\mathbb{K}[X]}
\makeatletter
\newcommand*{\ar}{\@ifstar\ard\arf}
\makeatother
\newcommand{\ard}{\begin{array}{l}}
\newcommand{\arf}{\end{array}}
\newcommand{\col}[2]{\begin{multicols}{2} #1 \columnbreak \\ #2 \end{multicols}}
\newcommand{\sumi}{\sum\limits_{i\in I}}
\newcommand{\prodi}{\prod\limits_{i\in I}}
\newcommand{\mc}[1]{\left\vert #1 \right\vert}
\newcommand{\ent}[1]{\llbracket #1 \rrbracket}
\newcommand{\scal}[1]{\left< #1 \right>}
\newcommand{\thm}[4]{\hspace*{1.5cm}\newtheorem*{#1}{#2}  \begin{minipage}{12.71cm} \begin{#1}  ${}$ \vspace*{0.15cm}\\ \hspace*{0.21cm} 			
    \begin{blockarray}{|l} #4 \end{blockarray} \end{#1}\label{#3} \end{minipage} \vspace*{-0.25cm} ${}$}
\newcommand{\traitd}{\begin{center} \rule{14cm}{0.75pt} \vspace*{-0.4cm}\\ \end{center}}
\newcommand{\trait}{\begin{center} ${}$ \vspace*{-0.8cm}  \\ \rule{14cm}{0.75pt} \end{center} ${}$ \\ }
\newcommand{\N}{{\mathds{N}}}
\newcommand{\Z}{{\mathds{Z}}}
\newcommand{\R}{{\mathds{R}}}
\newcommand{\C}{{\mathds{C}}}
\newcommand{\K}{{\mathds{K}}}
\newcommand{\Q}{{\mathds{Q}}}
\newcommand{\M}{{\mathcal{M}}}
\newcommand{\cont}{{\mathscr{C}}}
\newcommand{\cpm}{{\mathscr{C}^0_{pm}\big([a,b],\R\big)}}
\newcommand{\lin}{{\mathscr{L}}}
\newcommand{\GL}{{\mathcal{GL}}}
\newcommand{\A}{{\mathscr{A}}}
\newcommand{\Part}{{\mathcal{P}}}
\newcommand{\ston}{\underset{n}{\rightarrow}}
\newcommand{\stox}[1]{\underset{x\to #1}{\longrightarrow}}
\newcommand{\dd}{{\mathrm{d}}}
\newcommand{\limit}[2]{{\underset{#1 \rightarrow #2}{\lim}}}
\newcommand{\highlight}[1]{\colorbox{lightgray}{#1}}
\newcommand{\suminft}{\sum_{n=0}^{+\infty}}
\newcommand{\appli}[4]{\begin{blockarray}[t]{(ccc)} #1 & \hspace*{-0.25cm} \longrightarrow \hspace*{-0.25cm} & #3 \\ #2 & \hspace*{-0.25cm}\mapsto\hspace*{-0.25cm} & #4 \end{blockarray}}
\newcommand{\Esc}{{\mathcal{E}\big([a,b],\R\big)}}
\newcommand{\cm}[1]{\displaystyle{ #1 }}
\newcommand{\suite}[1]{( #1_n)_{n\geqslant 0}}
\newcommand{\CO}{\mathscr{C}^0}
\newcommand{\Cun}{\mathscr{C}^1}
\newcommand{\Ck}{\mathscr{C}^k}
\newcommand{\Cinf}{\mathscr{C}^{\infty}}
\newcommand{\un}{{\scriptsize (1)} }\newcommand{\deux}{{\scriptsize (2)} }
\newcommand{\trois}{{\scriptsize (3)} }\newcommand{\quatre}{{\scriptsize (4)} }
\newcommand{\fin}{{\LARGE $\star$ \hspace*{0.7cm} $\star$ \hspace*{0.7cm} $\star$}}
\newcommand{\prehilb}{(E,\scal{.,.})}
\newcommand{\norm}[1]{{\| #1 \|}}
\newcommand{\pfrac}[2]{\frac{\partial #1}{\partial #2}}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                               COVER
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\begin{titlepage}

\noindent
\titlefont ~\vspace{50pt}\\Mathématiques \\ \hspace*{150pt} Préparatoires I\par
\epigraph{Ce document est une synthèse du cours de mathématiques dispensé par M. Jean-François \textsc{Mallordy} 
    en classe préparatoire au lycée Blaise Pascal, Clermont-Ferrand en 2022-2023. Il s'agit d'un complément au cours de Maths Spé 
    et ne saurait en aucun cas y être un quelconque remplacement !} %
{\textit{Paris, 2024}}
\null\vfill
\vspace*{1cm}
\noindent
\hfill
\begin{minipage}{0.45\linewidth}
    \begin{flushright}
        \printauthor
    \end{flushright}
\end{minipage}
%
\begin{minipage}{0.02\linewidth}
    \vspace{-20pt}
    \rule{1pt}{55pt}
\end{minipage}
\titlepagedecoration
\end{titlepage}

\setcounter{tocdepth}{2}
\renewcommand{\thefootnote}{\fnsymbol{footnote}}
\renewcommand{\contentsname}{Table des matières - Première année}
\setcounter{minitocdepth}{4}
\renewcommand{\mtctitle}{Contenu}
\dominitoc

\setcounter{chapter}{-1}

\theoremstyle{plain}

\shorttoc{Chapitres}{0}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                          DOCUMENT START
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\chapter{Introduction}

    \input{../1_chapitres/ch1_0_fr}

\chapter{Ensembles et applications}

    \input{../1_chapitres/ch1_1_fr}

\chapter{Calculus}

    \input{../1_chapitres/ch1_2_fr}

\chapter{Nombres Complexes}

    \input{../1_chapitres/ch1_3_fr}
    
\chapter{Fonctions}

    \input{../1_chapitres/ch1_4_fr}
        
\chapter{Primitives et équations différentielles}

    \input{../1_chapitres/ch1_5_fr}

\chapter{Nombres réels et suites numériques}

    \input{../1_chapitres/ch1_6_fr}

\chapter{Fonctions d'une variable réelle}

    \input{../1_chapitres/ch1_7_fr}

\chapter{Arithmétique dans $\mathbf{Z}$}

    \input{../1_chapitres/ch1_8_fr}

\chapter{Structures algébriques usuelles}

    \input{../1_chapitres/ch1_9_fr}
    
\chapter{Calcul matriciel et systèmes linéaires}

    \input{../1_chapitres/ch1_10_fr}
        
\chapter{Polynômes et fractions rationnelles}

    \input{../1_chapitres/ch1_11_fr}

\chapter{Analyse asymptotique}

    \input{../1_chapitres/ch1_12_fr}
        
\chapter{Espaces vectoriels et applications linéaires}

    \input{../1_chapitres/ch1_13_fr}

\chapter{Matrices II}

    \input{../1_chapitres/ch1_14_fr}
    
\chapter{Groupe symétrique et déterminant}

    \input{../1_chapitres/ch1_15_fr}
        
\chapter{Intégration}

    \input{../1_chapitres/ch1_16_fr}

\chapter{Dénombrement}

    \input{../1_chapitres/ch1_17_fr}
        
\chapter{Probabilités}

    \input{../1_chapitres/ch1_18_fr}

\chapter{Espaces préhilbertiens réels}

    \input{../1_chapitres/ch1_19_fr.tex}

\chapter{Procédés sommatoires discrets}

    \input{../1_chapitres/ch1_20_fr.tex}

\chapter{Fonctions de deux variables}

    \input{../1_chapitres/ch1_21_fr.tex}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%                         Table of Contents
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

%\setcounter{tocdepth}{1}

%\renewcommand{\contentsname}{Table des Matières - Première année}
%\tableofcontents

\tableofcontents

\anothertoc{../2_main_fr/mathematic_basics_fr_2}{Table des matières - Deuxième année}{2}
    

\end{document}