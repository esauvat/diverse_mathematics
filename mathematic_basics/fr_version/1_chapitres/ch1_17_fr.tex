...
\minitoc
	...
	\subsection{Généralités}
		...
		\hspace*{0.5cm} Alors $F$ est fini et $\mc{F} \leqslant n$ }
		\vspace*{0.5cm} \\ \thm{ch18L1c}{Corollaire}{18-L1}{Si $E$ et $F$ sont des ensembles avec $F$ fini et $E\subset F$ \\
		\hspace*{0.5cm} Alors $E$ est fini et $\mc{E} \leqslant \mc{F}$ \\
		...
		\hspace*{15pt} {\small 1)} Si $f$ injective $\mc{f(E)} = \mc{E}$ et $ \mc{E} \leq \mc{F}$ \\
		\hspace*{15pt} {\small 2)} Si $f$ surjective $\mc{F} \leq \mc{E}$\\
		\hspace*{15pt} {\small 3)} Si $\mc{F} = \mc{E}$ alors $f$ est injective si et seulement si $f$ est surjective.}
		\vspace*{0.5cm} \\ \thm{ch18P2}{Propriété}{CardCompl}{Soit $E$ un ensemble fini et $A\in\mathcal{P}(E)$ alors $~\mc{\mathcal{C}_E A} = \mc{E} - \mc{A}$ }
		\vspace*{0.5cm} \\ \thm{ch18P2c}{Corollaire}{CardDiff}{Si $A$ et $B$ sont finis alors $A\setminus B$ est fini et $~\mc{A\setminus B} = \mc{A}-\mc{B}$}
		\vspace*{0.5cm} \\ \thm{ch18P3}{Proposition}{CardUnion}{ Si $A$ et $B$ sont finis alors $A\cup B$ est fini et $~\mc{A\cup B} ~=~ \mc{A} + \mc{B} - \mc{A\cap B}$}
	\subsection{Lemme des Bergers et principe des Tirroirs}
		${}$ \\ \thm{ch18P4}{Proposition}{CardPart}{Si $P$ est une partition de $E$ (cf -> \ref{def partition} ) alors $\mc{E} = \sum\limits_{X\in P} \mc{X}$} 
		\vspace*{0.5cm} \\ \thm{ch19th1}{Lemme des Bergers}{lemme bergers}{Soit $E,F$ deux ensembles finis et $f: E\rightarrow F$ telle que \\ $\exists p \in \mathbb{N}^* ~:~~\forall y\in F ~,~~ \mc{f_r^{-1} (\{y\} )} = p ~~$ alors $~~ \mc{E} ~=~ p\mc{F}$}
		\begin{proof}
		$~~\left( f_r^{-1} (\{y\} ) \right)_{_{y\in F}}$ est une partition de $E$ et on a alors\\
		$\mc{E} ~=~ \sum\limits_{y\in F} \mc{f_r^{-1} (\{y\} )} ~=~ \sum\limits_{y\in F} p ~=~ p\mc{F}$
		\end{proof}
		${}$ \\ \thm{ch19th2}{Principe des Tirroirs de Dirichlet}{princ.tirroirs}{Soit $E$ et $F$ deux ensemble finis de cardinaux respectifs $n$ et $p \in \mathbb{N}^*$\\$f:E\rightarrow F$ telle que s'il existe $k\in\mathbb{N} ~:~~n>kp ~$ alors $~\exists y\in F ~:~~ \mc{f_r^{-1} (\{y\} )} ~>~ k$}
		\begin{proof}
		On suppose que $\forall y\in F ~,~~ \mc{f_r^{-1} (\{y\} )} ~\leq ~k$ \\alors d'après le \underline{Lemme des Bergers} %(\ref{lemme bergers} ) 
        $~~\mc{E} ~\leq ~kp$ \end{proof} ${}$ 
	\subsection{Calcul sur les cardinaux}
		${}$ \\ \thm{ch18P5}{Proposition}{CalculCard}{Soit $E$ et $F$ deux ensembles finis alors \\
		\hspace*{0.5cm} $\rightarrow ~~ \mc{E\times F} ~=~ \mc{E} \times \mc{F}$ 
		\hspace*{51cm} $\rightarrow ~~ \mc{F^E} ~=~ \mc{F}^{\mc{E}}$} 
		\newpage
	...
	\section{Listes et Combinaisons}
		...
		$A_n^k = \mc{A_{k,n}} = \left\{ \begin{array}{cl} 0 & si~k>n \\ \frac{n!}{(n-k)!} & si~0\leq k\leq n \end{array} \right.$ } \\ \traitd
