...
\section{Norme}
	...
	\subsection{Généralités}
		...
		\theorem{lem}{Soit $(E,N)$ un espace vectoriel normé. \\
			Si $\forall (x,y)\in E^2$, $d(x,) = N(-y) $ alors $d$ est une distance sur E.
			}
...
\section{Suites}
		...
		\namedtheorem{Lemme 1 : Unicité de la limite}{
			Si \hspace*{0.2cm}
			$\ard u_n \underset{n}{\rightarrow} l_1 ~\in E \\u_n \underset{n}{\rightarrow} l_2 ~\in E \arf$ \hspace*{0.2cm} Alors $l_1 = l_2$
			}{UniqLimit}
		...
		\theorem{lem}{
			Soit $\left(u_n\right) _{_{n\in \mathbf{N} }} \in E^{\mathbf{N}} ,~l\in E $\hspace*{0.2cm} 
			Alors $u_n \underset{n}{\rightarrow} l ~\Leftrightarrow ~\norm{u_n - l} \underset{n}{\rightarrow} 0$
		}
		...
		\theorem{lem}{
			Toute suite $\left(u_n\right)_{_{n\geq 0}} \in E^{\mathbf{N}}$ convergente est bornée
		} \medskip \\
...
\section{Normes équivalentes}
	...
	\subsection{Cas de espaces de dimension fini}
		...
		\theorem{lem}{
			...
			Soit $\left(x_n\right)_{_{n\geq 0}} \in E^{\mathbf{N}}$ et $\alpha \in E$.
...
\section{Séries dans un K espace vectoriel de dimension finie}
		...
		\namedtheorem{Théorème : Reste d'une série convergente}{
			...
			et on note $S = \suminf u_n$ "limite de la somme".\\ Pour $n\in \mathbf{N}$ on pose $R_n = \sum_{k=n+1}^{+\infty} u_k$ 
			"reste d'ordre $n$". $\left\vert \ard \forall n\in \mathbf{N}$, $S=U_n + R_n \\ R_n \to 0 \arf \right.$
		}{RestSérie}
		...
		\traitd
		\paragraph{Convergence absolue} Soit $(u_n) ~\in E^{\mathbf{N}}$ on dit que $\sum u_n$ \underline{converge absolument} 
			\\ si $\sum \norm{u_n}$ converge.
		\trait 
		...
		\textsc{Attention !} Faux dans un EVN quelconque ! \medskip \\
...
\section{Complément sur les séries numériques}
	...
	\subsection{Règle de \textsc{Dalembert}}
		...
		\namedtheorem{Théorème : Règle de \textsc{Dalembert}}{
			...
			On suppose l'existence de $l \in \mathbf{R}\cup \{+\infty \}$ tel que $\abs{\frac{u_{u+1}}{u_n}} \to l$ \\
	...
	\subsection{Séries alternées}
		...
		\traitd
		\paragraph{Défnition}
			La série réelle $\sum u_n$ est dite \underline{alternée} si $\left\{\ard \forall n\in \mathbf{N} ,~u_n = (-1)^n \abs{u_n} \\
	...
	\subsection{Sommation des relations de comparaisons}
		...
		\namedtheorem{Théorème : Cas convergent}{
			...
			$\sum u_n$ et $\sum v_n$ converge et on poe $R_n =\sum{k=n+1}^{+\infty} u_n$ 
		...
		\namedtheorem{Théorème : Cas divergent}{
			Soit $(u_n) ,~(v_n) ~\in\mathbf{R}^{\mathbf{N}}$ 
			et $v_n\geq 0,~\forall n\geq n_0$ \\ On suppose que $\sum u_n$ et $\sum v_n$ diverge et on note $U_n = \sum_{k=0}^{n} u_n$ 
