...
\section{Norme}
	...
	\subsection{Normes euclidiennes}
		...
		\theorem{lem}{Si $E=\C^n ,~z=\left(z_1,\dots ,z_n\right) ~, ~N(z) = \sqrt{\sum_{k=1}^n\abs{z_k}^2}$ est une norme
			} \medskip
		
		\theorem{lem}{$E = \cont^0\left([a,b],\C\right)$ Soit $f\in E$ on pose \\
...
\section{Suites}
		...
		\theorem{lem}{
			Soit $\left(u_n\right) _{_{n\in \N }} \in E^{\N} ,~\ell\in E $\hspace*{0.2cm} 
		...
		\theorem{lem}{
			Soient $u_n ,~v_n \in E^{\N} ~$ et $ \lambda\in K$ si on a 
		...
		\theorem{lem}{
			Toute suite $\left(u_n\right)_{n\geq 0} \in E^{\N}$ convergente est bornée
		} \medskip
...
\section{Normes équivalentes}
	...
	\vspace{-15pt}
		...
		\theorem{lem}{
			Soit $N,N'$ des normes équivalentes sur $E$, $u\in E^{\N} ,~\ell\in E$ alors\\
	...
	\subsection{Cas de espaces de dimension fini}
		...
		\theorem{lem}{
			...
			Soit $\left(x_n\right)_{n\geq 0} \in E^{\N}$ et $\alpha \in E$.
		...
		\theorem{thm}{
			Soient $p,q,r \in \N^*$
			\hspace*{0.3cm}$\left\{\begin{array}{ll} A_n \underset{n}{\rightarrow} A & dans~\mathcal{M}_{p,q}\left(\R\right) \\ 
			B_n \underset{n}{\rightarrow} B & dans~\mathcal{M}_{q,r}\left(\R\right) \end{array}\right.$ \hspace*{0.3cm}
...
\section{Séries dans un $\K$ espace vectoriel de dimension finie}
		...
		\namedtheorem{Théorème : Reste d'une série convergente}{
			...
			Alors $\left\vert \ard \forall n\in \N$, $S=U_n + R_n \\ R_n \to 0 \arf \right.$
		}{RestSérie}
		...
		\theorem{lem}{
			Soit $(u_n) ,~ (v_n) ~\in E^{\N}$ et $\lambda\in \K$\\
		...
		\traitd
		\paragraph{Convergence absolue} Soit $(u_n) ~\in E^{\N}$ on dit que $\sum u_n$ \emph{converge absolument} 
			\\ si $\sum \norm{u_n}$ converge.
		\trait 
...
\section{Complément sur les séries numériques}
		...
		\rappel{Soit $z\in \C$ alors $\sum z^n$ CV $ \Rightarrow$ $\abs{z} <1$}
	...
	\subsection{Règle de \emph{Dalembert}}
		...
		\namedtheorem{Théorème : Règle de \emph{Dalembert}}{
			Soit $(u_n) \in\left(\C^*\right)^{\N}$ \\
			On suppose l'existence de $\ell \in \R\cup \{+\infty \}$ tel que $\abs{\frac{u_{u+1}}{u_n}} \to \ell$ \\
	...
	\subsection{Séries alternées}
		...
		\namedtheorem{Théorème : Critère spécial des série alternées}{
			...
			alors \highlight{$\sum u_n$ converge } et de plus, $\forall n\in \N$ \\
	...
	\subsection{Sommation des relations de comparaisons}
		...
		\namedtheorem{Théorème de \emph{Cesàro}}{
			Soit $(u_n) \in\R^{\N}$\\
			{\small 1)} Si $u_n \to \lambda$ avec $\lambda \in\R$, alors $\frac{1}{n+1} \sum_{k=0}^{n} u_k \to \lambda$ \\
